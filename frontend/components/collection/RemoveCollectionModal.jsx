  ...
  const handleRemoveCollectible = async () => {
    for (const collectible of toRemove) {
      const response = await fetch(
        `http://localhost:8080/collection/remove_collectible_from_collection?username=${router.query.user}&collectible_id=${collectible["id"]}`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <>
      <SuccessBar
        ...
        message="Successfully removed collectibles!"
      />
      ...
      >
        ...
        >
          ...
          >
            <FormControl sx={formStyle} onSubmit={handleRemoveCollectible}>
                ...
                <Paper elevation={0} sx={seriesContainerStyle}>
                  ...
                  {wooliesSeries.map((series, index) => (
                    <CollectiblesPreview
                      ...
                      collectibles={getCollected(series.collectibles)}
