...
def complete_trade():
    ...
    initiator_wishlist = (
        db_session.query(Wishlist).filter_by(account_id=initiator_account.id).first()
    )
    target_wishlist = (
        db_session.query(Wishlist).filter_by(account_id=target_account.id).first()
    )
    # Add the offered collectibles to the target's collection
    for collectible in trade_listing.offered_collectibles:
        # Modify the collection
        initiator_collection.collectibles.remove(collectible)
        target_collection.collectibles.append(collectible)
        # Modify the tradable
        if collectible in initiator_tradable.collectibles:
            initiator_tradable.collectibles.remove(collectible)
        # Modify the wishlist
        if collectible in target_wishlist.collectibles:
            target_wishlist.collectibles.remove(collectible)
            target_wishlist.number_of_items -= 1

    # Add the requested collectibles to the initiator's collection
    for collectible in trade_listing.requested_collectibles:
        # Modify the collection
        initiator_collection.collectibles.append(collectible)
        target_collection.collectibles.remove(collectible)
        # Modify the tradable
        if collectible in target_tradable.collectibles:
            target_tradable.collectibles.remove(collectible)
        # Modify the wishlist
        if collectible in initiator_wishlist.collectibles:
            initiator_wishlist.collectibles.remove(collectible)
            initiator_wishlist.number_of_items -= 1

    # Check all of the initiator's trade listings
    for collectible in trade_listing.offered_collectibles:
        initiator_trade_listings = db_session.query(TradeListing).filter(
            or_(
                TradeListing.initiator == initiator_account.username,
                TradeListing.target == initiator_account.username,
            ),
            or_(
                TradeListing.offered_collectibles.any(id=collectible.id),
                TradeListing.requested_collectibles.any(id=collectible.id),
            ),
            TradeListing.is_active == True,
            TradeListing.id != trade_listing.id,
        )
        for initiator_trade_listing in initiator_trade_listings:
            # Delete the trade listing
            db_session.delete(initiator_trade_listing)
        db_session.commit()

    # Check all of the target's trade listings
    for collectible in trade_listing.requested_collectibles:
        target_trade_listings = db_session.query(TradeListing).filter(
            or_(
                TradeListing.initiator == target_account.username,
                TradeListing.target == target_account.username,
            ),
            or_(
                TradeListing.offered_collectibles.any(id=collectible.id),
                TradeListing.requested_collectibles.any(id=collectible.id),
        ...
        for target_trade_listing in target_trade_listings:
            # Delete the trade listing
            db_session.delete(target_trade_listing)
        db_session.commit()
